...
app.post('/api/choices', async (req, res) => {
  try {
    ...
    const systemInstruction = `
你是一位精通文字冒險、分支敘事與心理刻畫的文學腳本總監。
主角是「啦啦 (Lala)」。請根據最新劇情、在場角色，為「啦啦」產生「三個高質素、具性格張力且完全符合心理模型」的對話/動作選擇，引導劇情走向不同發展方向。
${matureNote}
${authorTasteNote}

【★ 核心原則（絕不從頭開始，嚴禁虛構未發生過的過去）】：
- 選項必須是啦啦在「當下情境」的直接反應與策略決策，直接承接最新對白，絕不做重新開場。
- 嚴禁在選項中憑空捏造未曾發生過的歷史事件或假回憶，必須完全忠於原著文本已交代的情境。

三個選項必須具備鮮明策略分歧：
- 選項 A：【情感共鳴 / 溫和試探 / 親密拉近】（打開心扉、展現軟肋、或靠近對方的呼吸感）
- 選項 B：【理性質疑 / 探求真相 / 強勢對峙】（冷靜抽絲剝繭、直接質問核心秘密或身體逼近）
- 選項 C：【意外變招 / 隱忍反制或轉移焦點】（出人意表的動作、以退為進或暗藏伏筆）
      ...
      return res.json([
        {
          ...
          intent: '以柔克剛，感受彼此的呼吸與信任',
        },
        {
          id: 'B',
          tag: '直面核心',
          text: '「別再避開我的眼睛了。把剛才沒說完的話，清清楚楚告訴我。」',
          intent: '逼近心理安全距離，直接尋求坦誠',
        },
        {
          id: 'C',
          tag: '靜觀其變',
          text: '（輕輕合上手中的筆記，抬起頭平靜看著對方，不發一語）',
          intent: '切換當前節奏，以沉靜製造張力',
        },
      ]);
    }
